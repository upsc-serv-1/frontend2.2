...
export function buildHierarchicalPerformanceReport(questions: QuestionAttempt[]): HierarchicalPerformance {
  ...
  questions.forEach((question) => {
    ...
    if (question.attemptHour !== undefined) {
      ...
      if (!report.advanced.fatigue[h]) report.advanced.fatigue[h] = { total: 0, correct: 0 };
      report.advanced.fatigue[h].total++;
      if (isCorrect) report.advanced.fatigue[h].correct++;
    }
    ...
    if (question.difficultyLevel) {
      ...
      if (!report.advanced.difficulty[d]) report.advanced.difficulty[d] = { total: 0, correct: 0 };
      report.advanced.difficulty[d].total++;
      if (isCorrect) report.advanced.difficulty[d].correct++;
    }
    ...
    if (question.confidence) {
      ...
      if (!report.advanced.confidence[normalizedConfidence]) {
        report.advanced.confidence[normalizedConfidence] = { total: 0, correct: 0, incorrect: 0 };
      }
      report.advanced.confidence[normalizedConfidence].total++;
      if (isCorrect) report.advanced.confidence[normalizedConfidence].correct++;
      if (isIncorrect) report.advanced.confidence[normalizedConfidence].incorrect++;
    }

    if (isIncorrect) {
      const e = question.errorCategory || 'Other';
      // Map app ERROR_TYPES to internal groups
      ...
      if (!report.advanced.errors[group]) report.advanced.errors[group] = 0;
      report.advanced.errors[group]++;
    }
    ...
    if (!report.subjects[subjectName]) {
      report.subjects[subjectName] = { name: subjectName, ...createBaseStats(), sectionGroups: {} };
    }
    ...
    if (!subject.sectionGroups[sectionName]) {
      subject.sectionGroups[sectionName] = { name: sectionName, ...createBaseStats(), microTopics: {} };
    }
