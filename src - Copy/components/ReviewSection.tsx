...
import { AlertTriangle, Target, Activity, Clock, ShieldAlert, TrendingUp } from 'lucide-react-native';
...
export const ReviewSection = ({ testAttemptId }: ReviewSectionProps) => {
  ...
  const { loading, error, scoreData, hierarchicalPerformance, confidenceMetrics } = useSingleTestAnalytics(testAttemptId);
  const [sectionOrder, setSectionOrder] = useState<string[]>(DEFAULT_ANALYTICS_LAYOUT.review);

  useEffect(() => {
    loadAnalyticsLayout().then(layout => setSectionOrder(layout.review));
  }, []);
  ...
  if (error || !scoreData || !hierarchicalPerformance || Object.keys(hierarchicalPerformance.subjects).length === 0) {
    return (
      <View style={[styles.center, { padding: spacing.xl, marginTop: 100 }]}>
        ...
        <TouchableOpacity 
          style={{ marginTop: 32, padding: 16, backgroundColor: colors.primary, borderRadius: 12, width: '100%', alignItems: 'center' }}
          onPress={() => router.push(`/test/result/${testAttemptId}`)}
        >
          <Text style={{ color: '#fff', fontWeight: '800' }}>VIEW QUESTIONS DIRECTLY</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity 
        style={[styles.viewQuestionsBtn, { backgroundColor: colors.primary }]}
        onPress={() => router.push(`/test/result/${testAttemptId}`)}
      >
        <Text style={styles.viewQuestionsText}>EXPLANATION HUB</Text>
      </TouchableOpacity>
    </ScrollView>
  );
};
