...
export const RepoQuestionCard = ({ question, onUpdate, isZenMode }: RepoQuestionCardProps) => {
  ...
  const handleAddToFlashcard = async () => {
    ...
    try {
      const front = `${question.subject} - ${question.microTopic}\n\n${question.questionText}`;
      const back = `Correct Answer: ${question.correctAnswer}\n\nExplanation: ${question.explanation}`;
      const { error } = await supabase.from('flashcards').insert({ user_id: session.user.id, front, back });
      if (error) throw error;
      Alert.alert('Success', 'Question synced to flashcards!');
    } catch (err: any) {
      Alert.alert('Error', err.message);
    } finally {
      setLoadingAction(null);
