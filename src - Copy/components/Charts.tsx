...
export const PieChart = ({
  ...
  return (
    <View style={{ flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
      <View style={{ width: chartWidth, height: size + 120 }}>
        <Svg height={size + 120} width={chartWidth}>
          {total === 0 ? (
        ...
        <View style={[StyleSheet.absoluteFillObject, { width: chartWidth, height: size + 30, alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }]}>
    ...
    <View style={{ flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 12, paddingHorizontal: 20 }}>
        {data.slice(0, 6).map((item, i) => (
          <TouchableOpacity 
            key={item.tag} 
            style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: themeColors.surface, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: themeColors.border }}
            onPress={() => onPress && onPress(item.tag)}
          >
            <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: chartColors[i % chartColors.length] }} />
            <Text style={{ fontSize: 11, color: themeColors.textSecondary, fontWeight: '700' }}>
              {item.tag} ({total > 0 ? Math.round((item.count/total)*100) : 0}%)
            </Text>
    ...
    <View style={{ height, width: '100%', marginTop: spacing.md }}>
      <Svg height={height} width={screenWidth}>
        ...
        {labels.map((label, i) => (
          <SvgText
            key={i}
            x={paddingLeft + (i / (labels.length - 1 || 1)) * (chartWidth - paddingRight - 10) + 10}
            y={height - 10}
            fill={theme.textSecondary}
            fontSize="10"
            textAnchor="middle"
    ...
    <View style={{ height, width: '100%', marginTop: spacing.md }}>
      <Svg height={height} width={screenWidth}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((p, i) => (
          <G key={`y-${i}`}>
            <Path
              d={`M ${paddingLeft} ${chartHeight * (1 - p)} L ${screenWidth} ${chartHeight * (1 - p)}`}
              stroke={theme.border}
              strokeWidth="1"
              strokeDasharray="4 4"
            />
