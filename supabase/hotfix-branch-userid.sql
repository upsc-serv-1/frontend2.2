...
BEGIN
    ...
    -- 4. Add question_id to user_cards if missing (as TEXT to support all ID types)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='user_cards' AND column_name='question_id'
    ) THEN
        ALTER TABLE public.user_cards ADD COLUMN question_id TEXT;
    ELSE
        -- If it was created as UUID (from a previous failed hotfix run), convert to TEXT
        ALTER TABLE public.user_cards ALTER COLUMN question_id TYPE TEXT;
    END IF;
    ...
    -- 6. Add question_id to cards table if missing (as TEXT)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='cards' AND column_name='question_id'
    ) THEN
        ALTER TABLE public.cards ADD COLUMN question_id TEXT;
    ELSE
        -- Ensure it's TEXT
        ALTER TABLE public.cards ALTER COLUMN question_id TYPE TEXT;
    END IF;
